  ...
  if (ts.isVariableStatement(node)) {
    for (const decl of node.declarationList.declarations) {
      ...
      if (name === "a" || name === "b") {
        ...
        console.log("apparent.flags:", apparent.flags, "Intersection:", apparent.flags === ts.TypeFlags.Intersection);
        ...
        if (constituents) {
          for (let i = 0; i < constituents.length; i++) {
            ...
            console.log(`  [${i}] isNumber:`, (c.flags & ts.TypeFlags.Number) !== 0);
            console.log(`  [${i}] isObject:`, (c.flags & ts.TypeFlags.Object) !== 0);
            const typeStr = checker.typeToString(c).trim();
            console.log(`  [${i}] typeToString: "${typeStr}"`);
            console.log(`  [${i}] typeStr.toLowerCase() === "number":`, typeStr.toLowerCase() === "number");
            ...
            console.log(`  [${i}] brand props (rule check):`, brandProps.map(p => String(p.escapedName)));
          }
        }
        ...
        if (constituents && constituents.length > 1) {
          for (const constituent of constituents) {
            ...
            if ((constituent.flags & ts.TypeFlags.Number) !== 0 || typeStr.toLowerCase() === "number") {
              hasNumber = true;
              console.log("Found number constituent");
            } else if (constituent.getProperties().some(p => {
              const n = p.escapedName as string;
              return n === "_brand" || n === "__brand" || /Brand$/.test(n);
            })) {
              hasBrandObj = true;
        ...
        console.log("Result (hasNumber && hasBrandObj):", hasNumber && hasBrandObj);
      }
    }
  }
});
