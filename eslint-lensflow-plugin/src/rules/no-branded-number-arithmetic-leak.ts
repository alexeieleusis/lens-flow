...
function isBrandedNumber(checker: ts.TypeChecker, tsType: ts.Type): boolean {
  ...
  for (const constituent of constituents) {
    // Check for number primitive OR Number object wrapper
    // In intersection types, `number` may appear as the `Number` object type
    if ((constituent.flags & ts.TypeFlags.Number) !== 0) {
      hasNumber = true;
    } else if ((constituent.flags & ts.TypeFlags.Object) !== 0) {
      ...
      if (
        props.some((p) => {
          ...
          // Handle variations: _brand, __brand, ___brand, XBrand
