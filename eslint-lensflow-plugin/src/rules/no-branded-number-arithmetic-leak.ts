...
function isBrandedNumber(checker: ts.TypeChecker, tsType: ts.Type): boolean {
  // Try the raw type first, then fall back to apparent type
  let constituents = (tsType as ts.IntersectionType)?.types;
  if (!constituents || constituents.length < 2) {
    constituents = (checker.getApparentType(tsType) as ts.IntersectionType)?.types;
  }
  ...
  for (const constituent of constituents) {
    ...
    if (
      (constituent.flags & (ts.TypeFlags.Number | ts.TypeFlags.NumberLiteral)) !== 0 ||
