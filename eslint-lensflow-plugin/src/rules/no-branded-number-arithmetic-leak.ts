...
function isNumberType(checker: ts.TypeChecker, tsType: ts.Type): boolean {
  if ((tsType.flags & ts.TypeFlags.Number) !== 0) return true;
  return checker.typeToString(tsType).toLowerCase() === "number";
}
