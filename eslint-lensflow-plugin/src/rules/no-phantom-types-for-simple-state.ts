...
const URL = knowledgeUrl("usecases/UC01-invalid-states.md", "Antipattern 3 — Overusing phantom types");
...
export default createRule({
  ...
  meta: {
    ...
    messages: {
      phantomTypeOveruse:
        "Type '{{name}}<{{params}}>' uses phantom type parameter(s) constrained to literal union(s) for simple state. Use a plain literal union type instead. See: {{url}}",
    },
    schema: [],
  },
  defaultOptions: [],
