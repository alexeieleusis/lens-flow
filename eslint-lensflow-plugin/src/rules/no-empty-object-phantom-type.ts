  ...
  meta: {
    ...
    messages: {
      emptyObjectPhantomType:
        "Type alias '{{name}}' is an empty object literal ({}), providing no structural distinction between states. Brand the state marker instead (e.g., type {{name}} = Brand<\"{{name}}\">). See: {{url}}",
    },
    schema: [],
    fixable: undefined,
  },
