...
function serializeTypeNode(node: TSESTree.TypeNode): string {
  switch (node.type) {
    ...
    case "TSOptionalType":
      return `${serializeTypeNode((node as TSESTree.TSOptionalType).typeAnnotation)}?`;
    case "TSRestType":
      return `...${serializeTypeNode((node as TSESTree.TSRestType).typeAnnotation)}`;
