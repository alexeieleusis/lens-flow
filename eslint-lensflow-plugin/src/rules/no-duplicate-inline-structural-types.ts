...
function serializeTypeNode(node: TSESTree.TypeNode): string {
  ...
  if (raw.type === "TSParenthesizedType" && raw.typeAnnotation) {
    return serializeTypeNode(raw.typeAnnotation as TSESTree.TypeNode);
  }
  switch (node.type) {
    ...
    case "TSLiteralType": {
      ...
      if (lit.type === "UnaryExpression") {
        return `${lit.operator}${(lit.argument as TSESTree.Literal).value}`;
      }
