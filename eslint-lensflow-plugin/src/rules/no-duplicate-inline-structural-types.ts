...
function serializeTypeNode(node: TSESTree.TypeNode): string {
  // Handle TSParenthesizedType before the switch since it may not be in the TypeNode union
  if (node.type as string === "TSParenthesizedType") {
    const pn = node as unknown as { typeAnnotation: TSESTree.TypeNode };
    return serializeTypeNode(pn.typeAnnotation);
  }
