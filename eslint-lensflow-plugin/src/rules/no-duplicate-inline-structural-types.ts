...
function getTypeName(node: TSESTree.EntityName): string {
  ...
  /* @ts-expect-error -- all EntityName variants handled */
  return node.type;
}
  ...
  switch (node.type) {
    ...
    case "TSLiteralType": {
      ...
      if (lit.type === "UnaryExpression") {
        const val = lit.argument.type === "Literal" ? String(lit.argument.value) : "";
        return `${lit.operator}${val}`;
      }
      /* @ts-expect-error -- all literal variants handled */
      return lit.type;
    }
    case "TSUnionType":
