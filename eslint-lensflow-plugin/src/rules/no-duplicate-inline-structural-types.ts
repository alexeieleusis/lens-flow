...
function getTypeName(node: TSESTree.EntityName): string {
  ...
  /* #__PURE__ */ (node as never); throw new Error(`Unknown EntityName type: ${(node as TSESTree.BaseNode).type}`);
}
  ...
  switch (node.type) {
    ...
    case "TSLiteralType": {
      ...
      if (lit.type === "UnaryExpression") {
        ...
        const argStr = arg.type === "Literal" ? String(arg.value) : arg.type;
        return `${lit.operator}${argStr}`;
      }
      /* #__PURE__ */ (lit as never); throw new Error(`Unknown literal type: ${(lit as TSESTree.BaseNode).type}`);
    }
    case "TSUnionType":
