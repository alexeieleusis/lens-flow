...
function serializeTypeNode(node: TSESTree.TypeNode): string {
  if ((node.type as string) === "TSParenthesizedType") {
    return serializeTypeNode(((node as unknown) as { typeAnnotation: TSESTree.TypeNode }).typeAnnotation);
  }
  switch (node.type) {
    ...
    case "TSLiteralType": {
      ...
      if (lit.type === "UnaryExpression") {
        const raw = ((lit.argument as TSESTree.Literal).raw ?? String((lit.argument as TSESTree.Literal).value)) as string;
        return `${lit.operator}${raw}`;
      }
      if (lit.type === "TemplateLiteral") {
        return lit.quasis.map((q) => q.value.cooked ?? "").join("");
      }
      return (lit as TSESTree.BaseNode).type;
    }
    case "TSUnionType":
