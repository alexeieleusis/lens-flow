...
function serializeTypeNode(node: TSESTree.TypeNode): string {
  // Handle types that may not be in the TypeNode union in some versions
  const anyNode = node as any;
  if (anyNode.type === "TSParenthesizedType") return serializeTypeNode(anyNode.typeAnnotation);
  if (anyNode.type === "TSOptionalType") return `${serializeTypeNode(anyNode.typeAnnotation)}?`;
  if (anyNode.type === "TSRestType") return `...${serializeTypeNode(anyNode.typeAnnotation)}`;
  switch (node.type) {
    ...
    case "TSLiteralType": {
      ...
      if (lit.type === "UnaryExpression") {
        ...
        const val = arg.type === "Literal" ? String(arg.value) : "";
        return `${lit.operator}${val}`;
      }
      return "";
    }
    case "TSUnionType":
