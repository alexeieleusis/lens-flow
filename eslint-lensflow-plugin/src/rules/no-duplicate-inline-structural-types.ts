...
function serializeTypeNode(node: TSESTree.TypeNode): string {
  // Handle TSParenthesizedType which may appear in AST but isn't in the TypeNode union
  if ((node.type as string) === "TSParenthesizedType") {
    return serializeTypeNode((node as { typeAnnotation: TSESTree.TypeNode }).typeAnnotation);
  }
  switch (node.type) {
