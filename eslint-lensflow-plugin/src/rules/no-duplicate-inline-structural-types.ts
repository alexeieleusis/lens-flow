...
function serializeTypeNode(node: TSESTree.TypeNode): string {
  // Handle parenthesized types (not in TypeNode union for this typescript-eslint version)
  ...
  switch (node.type) {
    ...
    case "TSLiteralType": {
      ...
      if (lit.type === "UnaryExpression") {
        return `-${lit.argument.type}`;
      }
      // Enum member literals may appear as Identifier at runtime
      if ((lit as TSESTree.Node).type === "Identifier") {
        return (lit as TSESTree.Identifier).name;
      }
      return String((lit as { type: string }).type);
    }
    case "TSUnionType":
