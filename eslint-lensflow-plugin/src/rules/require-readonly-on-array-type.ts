  ...
  meta: {
    ...
    messages: {
      mutableArrayOnReadonlyProp:
        "Property '{{name}}' is marked `readonly` but uses mutable array type '{{type}}`. Use `readonly {{type}}` to prevent in-place mutation via push/splice. See: https://raw.githubusercontent.com/jpablo/vibe-types/refs/heads/main/plugin/skills/typescript/usecases/UC06-immutability.md",
      mutableArrayRefOnReadonlyProp:
        "Property '{{name}}' is marked `readonly` but uses mutable `Array<{{element}}>` type. Use `ReadonlyArray<{{element}}>` to prevent in-place mutation via push/splice. See: https://raw.githubusercontent.com/jpablo/vibe-types/refs/heads/main/plugin/skills/typescript/usecases/UC06-immutability.md",
    },
    schema: [],
    fixable: undefined,
  },
