...
function nonNullExprMatchesTarget(expr: unknown, objName: string, propName: string): boolean {
  const info = getObjAndProp(expr);
  if (info && (info.obj as { type?: string; name?: string }).type === "Identifier" && (info.obj as { name?: string }).name === objName && info.prop === propName) {
    return true;
  }
  if ((expr as { type?: string }).type === "ChainExpression") {
    return chainExpressionMatchesTarget((expr as { expression?: unknown }).expression, objName, propName);
  }
  return false;
}

function chainExpressionMatchesTarget(chain: unknown, objName: string, propName: string): boolean {
...
function pushChildNodes(stack: unknown[], node: object, visited: WeakSet<object>): void {
...
function pushChildOrArray(stack: unknown[], child: unknown, visited: WeakSet<object>): void {
  if (!child || typeof child !== "object") return;
  if (Array.isArray(child)) {
    for (const item of child) {
      if (item && typeof item === "object" && "type" in item && !visited.has(item)) {
        stack.push(item);
      }
    }
...
function hasNonNullAssertionInRhs(rhs: unknown, objName: string, propName: string): boolean {
  const visited = new WeakSet<object>();
  const stack: unknown[] = [rhs];
  while (stack.length) {
    const n = stack.pop();
    if (!n || typeof n !== "object" || visited.has(n) || !("type" in n)) continue;
    visited.add(n);
    if ((n as { type: string }).type === "TSNonNullExpression" && nonNullExprMatchesTarget((n as { expression?: unknown }).expression, objName, propName)) {
      return true;
    }
...
function stmtBlockContainsNode(stmt: unknown, node: unknown): boolean {
  return stmt === node || (typeof stmt === "object" && stmt != null && "type" in stmt && (stmt as { type: string }).type === "BlockStatement" && bodyOfBlockContains(stmt, node));
}

function nullGuardIfContainsNode(stmt: unknown, node: unknown, objName: string, propName: string): boolean {
  if (typeof stmt !== "object" || stmt == null || !("type" in stmt)) return false;
...
function bodyOfBlockContains(block: unknown, target: unknown): boolean {
  ...
  const b = block as { type: string; consequent?: unknown; alternate?: unknown; body?: unknown[] };
  if (b.type === "BlockStatement" || b.type === "IfStatement") {
    const body = b.type === "IfStatement" ? (b.consequent || b.alternate) : block;
    if (body && typeof body === "object" && "type" in body && (body as { type: string }).type === "BlockStatement") {
...
function isNullishValue(node: unknown): boolean {
  ...
  return (n.type === "Literal" && (n.value === null || n.value === undefined))
    || (n.type === "Identifier" && n.name === "undefined");
}

function objMatchesName(info: { obj: unknown; prop: string }, objName: string, propName: string): boolean {
  const obj = info.obj as { type?: string; name?: string };
  return obj.type === "Identifier" && obj.name === objName && info.prop === propName;
}
...
function matchesBinaryNullCheck(
  ...
  if (leftInfo && objMatchesName(leftInfo, objName, propName) && isNullishValue(t.right)) {
    return true;
  }
  if (rightInfo && objMatchesName(rightInfo, objName, propName) && isNullishValue(t.left)) {
    return true;
  }
  return false;
}
...
function matchesNullCheck(test: unknown, objName: string, propName: string): boolean {
  if (!test || typeof test !== "object" || !("type" in test)) return false;
  const t = test as { type: string; left?: unknown; right?: unknown; operator?: string; argument?: unknown };
